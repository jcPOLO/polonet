...
def mock_csv_bad():
    ...
    writer.writeheader()
    writer.writerow(
        {"hostname": "1.1.1.1", "platform": "ios", "serial_number": "12345"}
    )
    writer.writerow(
        {"hostname": "2.2.2.2", "platform": "nxos", "serial_number": "23456"}
    )
    writer.writerow(
        {"hostname": "3.3.3.3", "platform": "ios", "serial_number": "34567"}
    )
    csv_file.seek(0)
    return csv_file
...
def test_import_inventory_file_good():
    with pytest.raises(FileNotFoundError):
        bootstrap = Bootstrap(csv_file="not_found.csv")
        bootstrap.load_inventory()

    bootstrap = Bootstrap(csv_file=f"{dir_path}/static/inventory_ok.csv")
